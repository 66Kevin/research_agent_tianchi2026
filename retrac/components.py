...
def _is_data_inspection_failed(exc: Exception, msg: str) -> bool:
    ...
    lowered = msg.lower()
    return (
        "data_inspection_failed" in lowered
    ...
    async def _llm_ainvoke(
        ...
        **kwargs: Any,
        ...
        for llm_attempt_idx in range(RETRY_ATTEMPTS):
            attempt_no = llm_attempt_idx + 1
            attempt_started_at = _utc_now_iso()
            attempt_start = time.perf_counter()
            try:
                if model_cfg.timeout_s and model_cfg.timeout_s > 0:
                    resp: AIMessage = await asyncio.wait_for(
                        llm.ainvoke(msgs),
                        timeout=model_cfg.timeout_s,
                    )
                else:
                ...
                return {key: msgs + [resp], "llm_attempts": llm_attempts}
            except Exception as e:
                if isinstance(e, asyncio.TimeoutError):
                    msg = f"LLM request timed out after {model_cfg.timeout_s}s"
                else:
                    msg = str(e)
                ...
                if _is_data_inspection_failed(e, msg):
                    non_retry_msg = (
                        ...
                        f"[{llm_attempt_idx + 1}/{RETRY_ATTEMPTS}] : {msg}"
                    )
                ...
                error.append(f"LLM invocation failed on attempt [{llm_attempt_idx + 1}/{RETRY_ATTEMPTS}] : {msg}")
