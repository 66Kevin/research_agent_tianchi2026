...
### run the agent in streaming mode (single task by `task_id`)
```bash
...
If needed, override the source file:

```bash
uv run run.py --config retrac/deep_research.yaml --question 0 --question-source /path/to/tasks.jsonl
```

### run the agent in non-streaming mode (single task by `task_id`)
```bash
uv run run.py --config retrac/deep_research.yaml --question 0 --non-streaming
```

### run the agent in batch mode (JSONL tasks)
Each line in the JSONL file must include:
- `task_id` (integer)
- `task_question` (string)

```bash
uv run run.py --config retrac/deep_research.yaml --task-file /data/dataset2/Workshop/wangyueyi/test/question.jsonl
```

### log outputs
By default, logs are enabled and written to:

```text
retrac/logs/run_YYYYMMDD_HHMMSS/
```

For each task, two files are generated:
- `task_{task_id}_{YYYY-MM-DD-HH-MM-SS}.events.jsonl` (streaming event log)
- `task_{task_id}_{YYYY-MM-DD-HH-MM-SS}.json` (final structured log)

In batch mode, task failures do not stop the run; remaining tasks continue.
