...
if not sys.platform.startswith('win'):
    try:
        ...
        # Fallback to the built-in sqlite3 when pysqlite3 is unavailable.
        pass
import json
import os
...
class Manus:
    def __init__(self, plan_llm, act_llm, tool_llm, vision_llm):
        ...
        # Keep per-actor message histories for external structured logging.
        self.actor_message_history_sessions = {}
    ...
    def execute(self, question, output_format=""):
        ...
        while True:
            ...
            # Only trigger replanning when there are blocked steps.
            # Replanning on every loop often causes step churn and long runtimes.
            if self.plan.has_blocked_steps():
                ...
                print("No blocked steps, skip replanning")
    ...
    def execute_steps(self, question, ready_steps):
        ...
        def execute_step(step_index):
            ...
            try:
                ...
                actor_name = task_actor_agent.agent_instance.instance_name
                actor_history = list(task_actor_agent.history)
                result_queue.put((step_index, result, actor_name, actor_history))
            finally:
                semaphore.release()
        ...
        while not result_queue.empty():
            step_index, result, actor_name, actor_history = result_queue.get()
            results[step_index] = result
            self.actor_message_history_sessions[actor_name] = actor_history
