...
class ActToolkit:
    ...
    def mark_step(self, step_index: int, step_status: str=None, step_notes: str=None, **kwargs) -> str:
        ...
        try:
            ...
            notes_len = len(step_notes or "")
            notes_preview = (step_notes or "").replace("\n", " ").strip()
            if len(notes_preview) > 300:
                notes_preview = notes_preview[:300] + " ...<truncated>"
            result = f"Step {step_index}: step_status={step_status}, step_notes_summary={notes_preview}"
            print(f"Step {step_index} marked as {step_status}, notes_len={notes_len}")
            print(self.plan.format(with_detail=False))
            return result
