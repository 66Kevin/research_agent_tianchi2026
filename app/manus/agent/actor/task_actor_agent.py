...
class TaskActorAgent(BaseAgent):
    ...
    def act(self, question, step_index):
        ...
        try:
            max_iteration = getattr(self.agent_instance.template, "max_iteration", 4) or 4
            result = self.execute(self.history, step_index=step_index, plan=self.plan, max_iteration=max_iteration)
            ...
            if os.getenv("ENABLE_STEP_FACT_UPDATE", "").lower() in {"1", "true", "yes"}:
...
"""
        ...
        try:
            max_iteration = getattr(self.agent_instance.template, "max_iteration", 4) or 4
            result = self.execute(self.history, plan=self.plan, max_iteration=max_iteration)
            pass
            return result
        except Exception as e:
            print(traceback.format_exc())
