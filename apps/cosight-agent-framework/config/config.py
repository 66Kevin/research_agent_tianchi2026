...
# 加载.env文件到环境变量
# Keep externally provided env vars (e.g. runtime API keys) as higher priority.
...
# ========== 第二路由模型配置 ==========
def get_second_route_model_config() -> dict[str, Optional[str | int | float]]:
    """获取第二路由模型配置，如果缺少配置则退回默认"""
    second_api_key = os.environ.get("SECOND_ROUTE_API_KEY")
    second_base_url = os.environ.get("SECOND_ROUTE_API_BASE_URL")
    second_model_name = os.environ.get("SECOND_ROUTE_MODEL_NAME")

    if not (second_api_key and second_base_url and second_model_name):
        return get_model_config()

    max_tokens = os.environ.get("SECOND_ROUTE_MAX_TOKENS")
    temperature = os.environ.get("SECOND_ROUTE_TEMPERATURE")
    ...
    return {
        ...
        "max_tokens": int(max_tokens) if max_tokens and max_tokens.strip() else None,
        "temperature": float(temperature) if temperature and temperature.strip() else None,
        "proxy": os.environ.get("SECOND_ROUTE_PROXY")
    }
