...
from benchmarks.evaluators.eval_utils import verify_answer_for_datasets
...
def _task_worker(task_dict, cfg_dict, evaluator_kwargs):
    ...
    evaluator = GenericEvaluator(
        ...
        cfg=cfg,
        log_dir=evaluator_kwargs.get("log_dir"),
...
def is_policy_blocked_result(
    status: Optional[str],
    boxed_answer: Optional[str],
    final_judge_result: Optional[str] = None,
) -> bool:
    """Check whether a task result indicates provider policy blocking."""
    if status == "blocked":
        return True
    if final_judge_result == BLOCKED_BY_POLICY_MESSAGE:
        return True
    return boxed_answer == BLOCKED_BY_POLICY_MESSAGE
...
class BenchmarkEvaluator(ABC):
    ...
    def get_log_dir(self) -> Path:
        ...
        In worker processes spawned by multiprocessing, Hydra runtime state
        is not initialized. Use the explicit `log_dir` provided by parent
        process first, and only fallback to Hydra runtime when available.
        """
        if self.log_dir is not None:
            return self.log_dir
        self.log_dir = Path(hydra.core.hydra_config.HydraConfig.get().run.dir)
        return self.log_dir
    ...
    async def run_single_task(self, task: BenchmarkTask) -> BenchmarkResult:
        ...
        try:
            ...
            for attempt in range(1, self.pass_at_k + 1):
                ...
                if matching_logs:
                    ...
                    try:
                        with open(log_file) as f:
                            ...
                            attempt_result["error_message"] = log_data.get("error", "")
                            if log_data.get("final_boxed_answer"):
                                ...
                                elif is_policy_blocked_result(
                                    status=attempt_result["status"],
                                    boxed_answer=attempt_result["model_boxed_answer"],
                                    final_judge_result=log_data.get(
                                        "final_judge_result"
                                    ),
                                ):
                                    attempt_result["final_judge_result"] = (
                                        BLOCKED_BY_POLICY_MESSAGE
                                    )
                                    attempt_result["judge_type"] = BLOCKED_JUDGE_TYPE
                ...
                if (
                    ...
                    while format_retry_count <= max_format_retries:
                        try:
                            ...
                            if is_policy_blocked_result(
                                status=attempt_result.get("status"),
                                boxed_answer=attempt_result["model_boxed_answer"],
                                final_judge_result=attempt_result.get(
                                    "final_judge_result"
                                ),
                            ):
                                attempt_result["status"] = "blocked"
                                attempt_result["final_judge_result"] = (
                                    BLOCKED_BY_POLICY_MESSAGE
                                )
                                attempt_result["judge_type"] = BLOCKED_JUDGE_TYPE
                                attempt_result["is_correct"] = False
                                attempt_result["error_message"] = response or (
                                    "Blocked by provider safety policy"
                                )
                                break
                ...
                if attempt_result["status"] == "blocked":
                    print(f"    🚫 Attempt {attempt}: BLOCKED_BY_POLICY")
                elif (
                    attempt_result["model_boxed_answer"]
            ...
            if is_policy_blocked_result(
                ...
                boxed_answer=result.model_boxed_answer,
                final_judge_result=result.final_judge_result,
            ):
                result.status = "blocked"
                result.final_judge_result = BLOCKED_BY_POLICY_MESSAGE
                result.judge_type = BLOCKED_JUDGE_TYPE
            elif found_correct_answer:
...
class GenericEvaluator(BenchmarkEvaluator):
    ...
    def __init__(
        ...
        """
        super().__init__(
            data_dir=data_dir, benchmark_name=benchmark_name, cfg=cfg, log_dir=log_dir
        )
