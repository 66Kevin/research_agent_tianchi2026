...
def generate_localization_gate_prompt(
    ...
    if mode == "degraded":
        tool_budget_line = "You may use at most 1 tool call in total during this gate.\n"
        mode_line = (
            "You are now in the Degraded Pre-Summary Localization Gate.\n\n"
        )
        scope_line = (
...
def generate_localization_gate_result_prompt(
    ...
    mode_instructions = (
        "- This was a skipped gate. Do not claim that a verified localized form was found.\n"
        "- Use `localized_name_status: NOT_FOUND` unless the conversation already contained a clear verified localized form before the gate was skipped.\n"
        "- If the gate was skipped due to time, explain that in `notes` and use the best known verified original full name.\n"
        if gate_mode == "skip"
        else ""
    )
