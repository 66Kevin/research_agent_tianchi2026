...
from ..utils.prompt_utils import BLOCKED_JUDGE_TYPE, BLOCKED_BY_POLICY_MESSAGE
...
async def execute_task_pipeline(
    ...
    try:
        ...
        if task_status == "blocked":
            task_log.status = "blocked"
            task_log.final_judge_result = BLOCKED_BY_POLICY_MESSAGE
            task_log.judge_type = BLOCKED_JUDGE_TYPE
            task_log.error = blocked_reason or "Blocked by provider safety policy"
            task_log.trace_data["blocked_reason"] = blocked_reason or ""
        else:
